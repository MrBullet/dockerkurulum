...
function mount_storage() {
  read -r -p "ZFS Depolama Alanı (örn: /depolama/media) Yolu: " ZFS_PATH
  if [[ -n "$ZFS_PATH" ]]; then
    msg_info "ZFS Depolama Bağlanıyor"
    pct set $CTID -mp0 $ZFS_PATH,mp=/mnt/media
    msg_ok "ZFS Bağlandı: /mnt/media"
    msg_info "ZFS Klasör İzinleri Ayarlanıyor"
    # Privileged LXC'de root:root sahipliğini zorluyoruz
    $STD chown -R 0:0 $ZFS_PATH
    msg_ok "ZFS İzinleri Root:Root olarak ayarlandı."
  fi
}
